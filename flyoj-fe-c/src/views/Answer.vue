  ...
  <div class="page praticle-page flex-col">
    <div class="box_1 flex-row">
      <div class="group_1">
        ...
        <el-countdown
          v-if="examEndTime && new Date() < new Date(examEndTime)"
          class="exam-time-countdown"
    ...
    <div style="width: 100%;height: 100%;">
        <!-- 外层 splitpanes：左侧题目描述，右侧代码编辑区和执行结果 -->
      <splitpanes class="splitpanes-container default-theme"  @resize="handleResize">
        <!-- 左侧：题目描述 -->
          ...
          <div class="box_3 flex-col">
            ...
            <span class="question-limit">
              <div v-if="questionDetail.difficulty === 1">
                题目难度：简单 时间限制：{{ questionDetail.timeLimit }} ms 空间限制：{{ questionDetail.spaceLimit }} 字节
              </div>
              <div v-if="questionDetail.difficulty === 2">
                题目难度：中等 时间限制：{{ questionDetail.timeLimit }} ms 空间限制：{{ questionDetail.spaceLimit }} 字节
              </div>
              <div v-if="questionDetail.difficulty === 3">
                题目难度：困难 时间限制：{{ questionDetail.timeLimit }} ms 空间限制：{{ questionDetail.spaceLimit }} 字节
              </div>
            </span>
            <span class="question-content"  v-html="questionContent"></span>
            <!-- <span class="question-content" v-html="processedContent"></span> -->
             <!-- <vue-markdown 
              ...
              <div class="group_15 flex-row">
                <div class="section_1 flex-row">
                  <div class="section_3 flex-col">
                    ...
                    <span class="error-text" v-if="userQuestionResultVO.pass === 0">异常信息：{{ userQuestionResultVO.exeMessage }}</span>
                    <el-table
...
const md = new MarkdownIt(); // 初始化 Markdown 解析器
async function getQuestionDetail() {
  ...
  questionContent.value = md.render(res.data.content);
...
.splitpanes-container {
  height: calc(100vh - 100px); /* 根据页面布局调整高度 */
  width: 100%;
  // display: flex;
